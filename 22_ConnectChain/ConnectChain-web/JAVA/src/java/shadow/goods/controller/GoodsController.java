...
public class GoodsController extends SuperController{
	...
	@RequestMapping(value = "/find-goods", method = RequestMethod.POST, headers = "Content-Type=application/json;utf-8")
	public ResponseEntity<String> findGoodsController(@RequestBody String json) throws Exception {
		String result = "";
		ABox jsonBox = new ABox();
		jsonBox = jsonBox.jsonToABox(json);
		try {
			result = goodsService.findGoods(jsonBox).aBoxToJsonObject().toString();
	...
	@RequestMapping(value = "/sold-goods-list", method = RequestMethod.POST, headers = "Content-Type=application/json;utf-8")
	...
	@RequestMapping(value = "/request-wish", method = RequestMethod.POST, headers = "Content-Type=application/json;utf-8")
	public ResponseEntity<String> requestWishController(@RequestBody String json) throws Exception {
		String result = "";
		ABox jsonBox = new ABox();
		jsonBox = jsonBox.jsonToABox(json);
		try {
			result = goodsService.requestWish(jsonBox).aBoxToJsonObject().toString();
		} catch (Exception e) {
			e.printStackTrace();
			return new ResponseEntity<String>(result, HttpStatus.SERVICE_UNAVAILABLE);
		}
